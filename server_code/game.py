...
welcome_message += "\nCommands are case-insensitive. 'start' to start a new game or resume an existing one. Happy gaming!\n"
...
class Game:
	def __init__(self, player_conn, player_addr, login_time, db_query_q, db_resp_q, db_query_lock):
		...
		self.player_uuid = ""	# start with an empty string
	...
	def _handle_id(self):
		...
		self._sendmsg("Write 'new' to set a new player ID (case-sensitive). Or enter your existing ID to resume playing.\n")
		while True:	
			...
			if user_input == "new":
				...
				self._show_command_prompt()
				input_id = self._handle_user_input()
	...
	def _handle_start(self):
		...
		for each_task in PiDayGame.tasks[self.current_task-1:]:
			...
			if self.task_input_handler(each_task) == "end":
	...
	def _handle_user_input(self):
		user_input = str(self.player_conn.recv(1024), 'utf-8').lower().strip()
	...
	def _show_leaderboard(self):
		...
		for each_player in player_data:
			...
			leaderboard += "{:<20}{:^10}{:^30}{:>}\n".format(each_player[0][:20], player_status, last_login, each_player[3])
	...
	def start_game(self):
		...
		while True:
			...
			if user_input == "start":
				self._handle_start()
				# redisplay the menu after the game ends
				self._show_welcome_message()
